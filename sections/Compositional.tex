...
Thus, the compilation system generates a complete \gls{vhdl} project structure that includes several key components: a main \gls{vhdl} file serving as the top-level entity with the full neural network implementation; a component library containing fixed-point arithmetic functions and utilities; individual \gls{vhdl} files for each activation function; and, optionally, a result processing block for classification tasks. 

Listing \ref{lst:usage_example} shows a basic usage example of the tool in Python. In the listing, line 2 imports the \texttt{ToVHDL} class from the \texttt{nn2vhdl} package. Lines 3 and 4 import the \texttt{Model} and \texttt{Input} classes from the \texttt{tensorflow.keras.models} and \texttt{tensorflow.keras.layers} packages, respectively. Lines 7-12 create a simple neural network with three layers and a sigmoid activation function. Lines 20 and 21 instantiate the models. Line 24-29 use the \texttt{ToVHDL} function to convert the model to \gls{vhdl} code. The function takes a list of models, a list of model names, a list of connections between the models, a list of classes, and a maximum number of inputs. The function returns a folder with the \gls{vhdl} code for the model. The function also accepts model weights and architecture information instead of the model itself if needed.
